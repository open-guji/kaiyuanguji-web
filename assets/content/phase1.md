...
少数专业的出版机构和专家学者会通过大量的人工校对，来保证数字化的准确性，但这个过程耗时耗力，且难以保证100%的准确性。我们希望找到（或开发）一套开源、现代化的工具，是的所有爱好者能够参与古籍的数字化，实现古籍的完美还原。
...
| **古籍还原度** | 低：upTeX只支持日文竖排。古籍的各种版式需要重新开发 | 极高：可利用 Lua 读取外部坐标文件，实现像素级定位。 | 一般：适合排纯文字内容，难以处理复杂的影印本版框复刻。 |
...
综上，采用类似LuaTeX-ja的方案（为日文设计），而为中文古籍实现所需要的功能，是最可行的方案。
